...
void SpecialGameScene::onExit() {
	Scene::onExit();
	User::inGame = false;
}

void SpecialGameScene::runCommand(SpecialGameScene* s, string command) {
	...
	if (op == "setscore") {
		...
		if (nums[1] < 0 || nums[1] >= 2)return;
		s->nowScoreA[nums[1]] = nums[0];
		s->updateScore();
	}
...
void SpecialGameScene::initComponents() {
...
#pragma endregion
#pragma region SliderBlueScore, 150
	{
		SliderBlueScore = Slider::create();
		SliderBlueScore->loadBarTexture(PICTURE_SLIDERBAR_BACK_PATH);
		SliderBlueScore->loadProgressBarTexture(PICTURE_SLIDERBAR_PRESS_PATH);
		SliderBlueScore->setPosition({ 250, 480 - 18 });
		SliderBlueScore->setRotation(-90);
		SliderBlueScore->setTag(150);
		SliderBlueScore->setMaxPercent(100);
		SliderBlueScore->setTouchEnabled(false);
		addChild(SliderBlueScore, 5);
	}
#pragma endregion

#pragma region SliderRedScore, 151
	{
		SliderRedScore = Slider::create();
		SliderRedScore->loadBarTexture(PICTURE_SLIDERBAR_BACK_PATH);
		SliderRedScore->loadProgressBarTexture(PICTURE_SLIDERBAR_PRESS_PATH);
		SliderRedScore->setPosition({ 115, 480 - 18 });
		SliderRedScore->setRotation(-90);
		SliderRedScore->setTag(151);
		SliderRedScore->setMaxPercent(100);
		SliderRedScore->setTouchEnabled(false);
		addChild(SliderRedScore, 5);
	}
#pragma endregion
...
#pragma region LabScore, 170,171
	{
		for (int i = 0; i < 2;i++) {
			LabScore[i] = Label::create("0", "fonts/Marker Felt.ttf", 30);
			LabScore[i]->setPosition(182 + 62*(i?-1:1),260+7);
			LabScore[i]->setTextColor({ 255,255,255,255 });
			LabScore[i]->enableOutline({ 0,0,0,255 }, 3);
			LabScore[i]->setTag(170+i);
			addChild(LabScore[i], 5);
		}
	}
#pragma endregion
...
#pragma region BtnRankList, 202
	{
		...
		BtnRankList->setPosition({ 157,78 });
		BtnRankList->setTag(202);
		addChild(BtnRankList, 5);

		BtnRankList->addClickEventListener(CC_CALLBACK_1(SpecialGameScene::RankCallBack, this));
	}
#pragma endregion
#pragma region BtnPrompt, 203
	{
		...
		BtnPrompt->setPosition({ 247,78});
		BtnPrompt->setTag(203);
		addChild(BtnPrompt, 5);

		BtnPrompt->addClickEventListener(CC_CALLBACK_1(SpecialGameScene::PromptCallBack, this));
	}
#pragma endregion
...
#pragma region EditBoxConsole, 401
	{
		class tmpDelegate : public cocos2d::ui::EditBoxDelegate {
			...
			virtual void editBoxTextChanged(cocos2d::ui::EditBox* editBox, const std::string& text) {
				...
				for (auto elm : text) {
					if (elm == '\n') {
						runCommand((SpecialGameScene*)editBox->getParent(), str);
						editBox->setText("");
					}
					str += elm;
				}
...
void SpecialGameScene::SettingCallBack(Ref*) {
	...
	setting->setCallbackFunc(this, callfuncN_selector(SpecialGameScene::settingSaveCallBack));
	this->addChild(setting, 100);
}

void SpecialGameScene::RankCallBack(Ref*) {
	auto rank = ScoreLayer::create(1);
	addChild(rank, 100);
}

void SpecialGameScene::PromptCallBack(Ref*) {
	if (!boardOperatable())return;
