#ifndef __LEVELGAMESCENE_SCENE_H__
#define __LEVELGAMESCENE_SCENE_H__
...
class LevelGameScene : public cocos2d::Scene
{
public:
	bool init() override;
	...
	static LevelGameScene* create(int, int);

	static void runCommand(LevelGameScene* s, string command);
	/*设置分数*/
	void setLevelScore(int, int);
	/*设置交换次数*/
	void setLevelSwapCount(int, int);
	/*关卡*/
	int world;
	int level;
private:
	...
	/*static const*/ int SWAP_COUNT = 5;

	bool boardLocked = true;

	/*一星 二星 三星*/
	bool to70Percent = false;
	bool to90Percent = false;
	bool to100Percent = false;
	/*根据关卡设置最高分数*/
	const int levelScore[4][5] = {
		{5000, 10000, 15000, 20000, 25000},
		{10000, 20000, 30000, 40000, 50000},
		{15000, 30000, 45000, 60000, 75000},
		{20000, 40000, 60000, 75000, 90000}
	};
	/*根据关卡设置交换次数*/
	const int levelSwap[4][5] = {
		{15, 25, 35, 45, 55},
		{30, 40, 50, 60, 70},
		{40, 52, 64, 76, 88},
		{60, 70, 80, 90, 100}
...
#pragma region CONST_STRING
	...
	// star bar
	const string PICTURE_STAR_BAR_PATH = "../Resources/texture/GameScene/star_bar.png";
	// star
...
#pragma region NORMAL_NODE
	...
	ImageView* PicBoard;
	ImageView* PicCover;
	ImageView* PicStarBarBac[3];
	ImageView* PicStar[3];
	Label* LabelStarScore[3];
	Button* BtnBarBack;
	Slider* SliderScore;
	Label* LabelSwap;
	Label* LabelScore;
...
#endif // __LEVELGAMESCENE_SCENE_H__
