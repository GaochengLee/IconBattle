...
Actor* Actor::create(int type,int func,pii pos) {
	...
	if (actor) {
		...
		actor->setGem(Sprite::createWithSpriteFrameName(GEM_N[type]));
...
void myGame::Actor::moveToThenBack(pii posb) {
	auto action = Sequence::create(
...
void myGame::Actor::dropTo(pii pos) {
	auto action = Sequence::create(
